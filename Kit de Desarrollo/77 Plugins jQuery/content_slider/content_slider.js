/*
Content Slider 1.0.1

Copyright (c) 2013 Br0 (www.shindiristudio.com)

Project site: http://www.codecanyon.net/
Project demo: http://www.shindiristudio.com/content_slider/
*/
...
// ----------------- Content Slider main class ---------------------------
	function content_slider(element, options) {
		...
		if (!this.options.top_offset)
			this.options.top_offset = Math.floor (this.options.big_pic_width/2) + this.options.big_border+1;
	...
	content_slider.prototype = {
		...
		public_go_left: function() {
			if (this.is_auto_play==1 && this.dismiss_auto_play==1) return;
		...
		_resize: function() {
			...
			if (ww<290) ww=290;
			if (this.options.max_shown_items==1) {
				ww=this.options.big_pic_width+63;
			}
		...
		create_html: function () {
			...
			var i, img, real_i;
			var html_code = '<div class="'+this.options.container_class+'"><div class="image_more_info"><a href="#"><img alt="" src="images/more.png"></a></div>';
			this._start = -1;
		...
		_create_arrows : function () {
			...
			if (this.options.hv_switch) {
				if (this.options.border_on_off==0 || this.options.use_thin_arrows==1) {
					html = '<div class="circle_slider_nav_left"><img src="'+urlBase+'js/contentSlider/images/left_vertical2.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+urlBase+'js/contentSlider/images/right_vertical2.png" alt="right" /></div>';
				} else {
					html = '<div class="circle_slider_nav_left"><img src="'+urlBase+'js/contentSlider/images/left_vertical.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+urlBase+'js/contentSlider/images/right_vertical.png" alt="right" /></div>';
				}
			} else {
				if (this.options.border_on_off==0 || this.options.use_thin_arrows==1) {
					html = '<div class="circle_slider_nav_left"><img src="'+urlBase+'js/contentSlider/images/left2.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+urlBase+'js/contentSlider/images/right2.png" alt="right" /></div>';
				} else {
					html = '<div class="circle_slider_nav_left"><img src="'+urlBase+'js/contentSlider/images/left.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+urlBase+'js/contentSlider/images/right.png" alt="right" /></div>';
				}
			}
			return html;
		},
		...
		_show_arrows : function () {
			...
			if (this.options.hv_switch) {
				...
				if (this.options.border_on_off==0 || this.options.use_thin_arrows==1) right_end=6;
		...
		_set_arrows_events : function () {
			this.$left_arrow.click ($.proxy(function (e){e.preventDefault(); this.right_clicked();}, this));
			this.$right_arrow.click ($.proxy(function (e){e.preventDefault(); this.left_clicked();}, this));
			this.$left_arrow.mouseup ($.proxy(function (e){e.preventDefault(); this._arrow_mouse_up();}, this));
			this.$right_arrow.mouseup ($.proxy(function (e){e.preventDefault(); this._arrow_mouse_up();}, this));
			this.$left_arrow.on ('touchstart', $.proxy(function (e){e.preventDefault(); this.touch_event=1; this._arrow_mouse_up();}, this));
			this.$right_arrow.on ('touchstart', $.proxy(function (e){e.preventDefault(); this.touch_event=2; this._arrow_mouse_up();}, this));
			this.$left_arrow.mouseleave ($.proxy(function (){this._arrow_mouse_leave();}, this));
			this.$right_arrow.mouseleave ($.proxy(function (){this._arrow_mouse_leave();}, this));
			this.$left_arrow.mousedown ($.proxy(function (e){e.preventDefault(); this._arrow_mouse_down();}, this));
			this.$right_arrow.mousedown ($.proxy(function (e){e.preventDefault(); this._arrow_mouse_down();}, this));
			this.$left_arrow.on ('touchend', $.proxy(function (e){e.preventDefault(); this._arrow_mouse_down();}, this));
			this.$right_arrow.on ('touchend', $.proxy(function (e){e.preventDefault(); this._arrow_mouse_down();}, this));
		},
			...
			if (this.show_mouse_move) {	
				...
				if (Math.abs(this.math.sum_movement) == 0 && !this.was_gone) {
					...
					if (this.options.hv_switch)
						...
						var pos = this.math._change_master_position_by_x ((e.pageX - this.offset)+this.minus);
		...
		start_auto_play: function() {
			...
			this.dismiss_auto_play=0;
			this.is_auto_play=1;
			if (this.options.auto_play_direction==1) this.timeout_autoplay_handler=setInterval(function(){self.public_go_left();}, self.options.auto_play_pause_time);
			else this.timeout_autoplay_handler=setInterval(function(){self.public_go_right();}, self.options.auto_play_pause_time);
		},
	...
	position.prototype = {
		...
		_calculate_arrows_positions : function () {
			...
			var arrow_y_position = this.top_offset - Math.ceil (this.arrow_height/2);
			...
			if (this.parent_this.options.border_on_off==0 || this.parent_this.options.use_thin_arrows==1) {
				first_arrow_x_position-=big_border+5;
			...
			if (this.sum_movement <= 0)
			{
				...
				if (new_n == this.mid_elem) {
					...
					if (!operation || this.mode == 2) {
						...
						new_y_pos = this.top_offset - Math.floor(this.small_pic_height/2) - (((this.master_element_width - this.small_pic_height)/2)*right_ratio) - this.small_border - (this.big_border - this.small_border)*right_ratio;
					} else {
						new_size = 0;
						new_y_pos = this.top_offset - (Math.floor(this.small_pic_height/2)) - this.small_border;
					}
				...
				if (new_n == this.mid_elem+1) {
					...
					if (!operation || this.mode == 2) {
						new_size = left_ratio;
						new_y_pos = this.top_offset - Math.floor(this.small_pic_height/2) - (((this.master_element_width - this.small_pic_height)/2)*left_ratio) - this.small_border;
					} else {
						new_size = 0;
						new_y_pos = this.top_offset - (Math.floor(this.small_pic_height/2)) - this.small_border; 
					}
